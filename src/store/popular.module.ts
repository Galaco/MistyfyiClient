...
import { FETCH_POPULAR_END, FETCH_POPULAR_START } from "./mutations.type"
...
const mutations = {
  ...
  [FETCH_POPULAR_END](state: State, resp: ApiResponse) {
    state.popularItems = new Popular(
      ...
      resp.body.maps.map(
        (item: any) => new HistoryItem(item.map_name, item.dateCreated)
      ),
      resp.body.mapNames.map(
        (item: any) =>
